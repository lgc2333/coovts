...
class ErrorID(IntEnum):
    ...
    """
    Returned when a request requires a certain permission that has not been granted for
    this plugin.
    """
    ...
    HotkeyQueueFull = 200
    """
    Only a certain amount of hotkey activations can be queued until you have to wait for
    ...
    """
    For example when trying to load a model and the user has certain config windows open
    that temporarily prevent that
    """

    HotkeyUnknownExecutionFailure = 206
    HotkeyExecutionFailedBecauseLive2DItemNotFound = 207
    """
    When triggering hotkeys in Live2D items, this will be returned if the requested
    Live2D item has not been found.
    """

    HotkeyExecutionFailedBecauseLive2DItemsDoNotSupportThisHotkeyType = 208
    """
    Only a subset of hotkey actions are supported in Live2D items, for example
    Expression hotkeys.
    """
    ...
    """
    This is usually because the user has menus open that prevent items from being
    loaded.
    """
    ...
    """
    Invalid values for custom data field (customDataBase64). Happens when data is
    provided here but it's no valid JPG/PNG data or image dimensions are invalid.
    """
    ...
    """
    Auto-stop frames indices have to be between 0 and the last frame index of the
    animated item.
    """
    ...
    """
    You can set stuff like transparency and brightness for normal PNG/JPG items, but
    nothing animation-related.
    """
    ...
    """
    Cannot load preset and individual config values with one request. Do one or the
    other.
    """
    ...
    """
    Tried to configure a restricted effect but user doesn't have those effects
    enabled/allowed.
    """
    ...
    """
    Shares its value with `EVENT_OFFSET`, so Python treats it as an alias of that member
    (`ErrorID.__members__` has both names, iteration yields one).
    """
