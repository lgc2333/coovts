...
- **进出 VTS 只有两条路：** 请求（`call_api` / `send_request`）和事件（一个声明出来的事件，也就是订阅
  加上它的 handler）。
- **库不镜像 VTS 的状态。** 模型列表、参数值都按需现问；库里真正保留的本地状态是连接状态机，以及你声明
  过的事件——每次重连之后它会重新订阅。
...
| 4    | [事件](./04-events.md)              | 声明出来的事件与订阅、handler 的关系，分发语义 |
